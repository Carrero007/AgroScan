...
FONTE (obrigatório): fonteNome=instituição/base técnica que embasa o diagnóstico e tratamento (priorize SEMPRE Embrapa — ex: 'Embrapa Hortaliças'; se usar outra referência agronômica real, coloque o nome real dela, nunca genérico). fonteUrl=link real e específico dessa fonte (priorize domínio embrapa.br: sistemas de produção, artigo técnico ou página da cultura/doença; se não souber URL exata da Embrapa, use a URL real da fonte técnica utilizada — nunca invente link).
RECOMENDAÇÃO PROFISSIONAL (obrigatório condicional): se confianca < 95, preencha recomendacaoProfissional com frase curta recomendando consultar um agrônomo de confiança em caso de dúvida antes de aplicar qualquer tratamento. Se confianca >= 95, recomendacaoProfissional deve ser string vazia """".";
